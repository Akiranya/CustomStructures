        ...
        plugin.getStructureHandler().getStructureDatabaseHandler().get().findNearby(nearbyStructuresRequest)
            .thenAccept(response -> {
                ...
                p.sendMessage(ChatColor.translateAlternateColorCodes('&',
                        "&aNearby Structures (&c%s&a, Limit &c%d&a):".formatted(
                            finalNearbyStructuresRequest.hasName() ? finalNearbyStructuresRequest.getName() : "All Structures",
                ...
                for (NearbyStructuresResponse.NearbyStructureContainer nearbyStructure : response.getResponse()) {
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&',
                        "&aFound structure &6%s &aat &6%s&a, &6%s&a, &6%s&a (&6%.2f &ablocks away)!".formatted(
