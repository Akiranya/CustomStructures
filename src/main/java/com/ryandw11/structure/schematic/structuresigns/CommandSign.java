...
public class CommandSign extends StructureSign {
    ...
    public boolean onStructureSpawn(@NotNull Location location, @NotNull Structure structure) {
        ...
        if (!hasArgument(0)) {
            plugin.getLogger().warning("Invalid command configuration on a structure sign! (%s)".formatted(structure.getName()));
            return true;
        }
        ...
        if (commands != null) {
            ...
            plugin.getLogger().warning("Unable to execute command group '%s', no configuration found!".formatted(getStringArgument(0)));
        }
